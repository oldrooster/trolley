...
import { CheckCircle, XCircle, Loader, Users, Plus, Pencil, Trash2, X, Upload, ChevronDown, ChevronRight } from 'lucide-react'
      ...
      <div className="card overflow-hidden">
        ...
        >
          <div>
            ...
            <p className="text-xs text-stone-400 mt-0.5">Powers recipe parsing, receipt extraction, and image generation.</p>
          </div>
          {aiOpen ? <ChevronDown className="w-4 h-4 text-stone-400 shrink-0" /> : <ChevronRight className="w-4 h-4 text-stone-400 shrink-0" />}
        </button>

        {aiOpen && (
          <div className="px-5 pb-5 pt-4 space-y-4 border-t border-stone-100 dark:border-stone-800">
            ...
            {provider === 'vertex' && (
              <div className="space-y-3">
                ...
                <div>
                  ...
                  />
                  <p className="text-xs text-stone-400 mt-1">Path inside the container where your JSON key is mounted. Leave blank to use Application Default Credentials.</p>
                </div>
              </div>
            )}
            ...
            {testResult && (
              <div className={`flex items-start gap-3 p-4 rounded-xl border ${testResult.ok ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                {testResult.ok
            ...
            <div className="bg-stone-50 dark:bg-stone-800 rounded-xl p-4 space-y-2">
              ...
              <pre className="text-xs text-stone-500 dark:text-stone-400 overflow-x-auto whitespace-pre">{`environment:
...
provider === 'gemini' ? `  GEMINI_API_KEY: your-api-key` : ''}`}
              ...
              <p className="text-xs text-stone-400">Environment variables are used as defaults. Values saved above take priority.</p>
            </div>
          </div>
        )}
      </div>
